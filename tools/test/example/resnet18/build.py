...
def copy_files_for_target():
    """拷贝文件到build目录"""
    print("=" * 60)
    print("拷贝文件到 build/generic 目录")
    print("=" * 60)
    
    # 确保build/generic目录存在
    build_dir.mkdir(parents=True, exist_ok=True)
    
    # 1. 拷贝resnet18目录的文件到build/generic
    ...
    # 2. 拷贝runtime/ops/generic目录到build/generic/ops/generic
    ...
    if runtime_ops_dir.exists():
        ...
        print(f"  已拷贝整个目录: runtime/ops/generic -> build/generic/ops/generic")
    else:
        print(f"  错误: 源目录不存在 {runtime_ops_dir}")
        return False
    
    # 3. 检查并从output目录拷贝模型文件（如果build/generic中不存在）
    print(f"\n[3/4] 检查模型文件...")
    output_dir = project_root / "output"
    model_files = ["resnet18.c", "resnet18.h"]
    for filename in model_files:
        src_file = output_dir / filename
        dst_file = build_dir / filename
        if not dst_file.exists() and src_file.exists():
            shutil.copy2(src_file, dst_file)
            print(f"  从output拷贝: {filename}")
        elif dst_file.exists():
            print(f"  已存在: {filename}")
        else:
            print(f"  警告: 模型文件不存在 {src_file}")
    
    # 4. 检查并从output目录拷贝权重目录（如果build/generic中不存在）
    src_weights_dir = output_dir / "weights"
    dst_weights_dir = build_dir / "weights"
    if not dst_weights_dir.exists() and src_weights_dir.exists():
        shutil.copytree(src_weights_dir, dst_weights_dir)
        print(f"  从output拷贝: weights/ 目录")
    elif dst_weights_dir.exists():
        print(f"  已存在: weights/ 目录")
    else:
...
def check_dependencies(use_build_dir=False):
    ...
    # 检查build目录下的模型代码
    model_c = current_build_dir / "resnet18.c"
    model_h = current_build_dir / "resnet18.h"
...
def compile_test(use_build_dir=False):
    ...
    source_files = [
        ...
        str(current_build_dir / "resnet18.c"),
    ]
