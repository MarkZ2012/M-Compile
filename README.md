...
│   │   ├── model.c/h         # 模型定义
│   │   └── *_test.c/exe      # 测试程序
│   ├── mobilenetv2_generic/   # MobileNetV2 通用版本
│   ├── mobilenetv2_quantized/ # MobileNetV2 量化版本
│   └── quantized/            # ResNet18 量化版本
│
├── calib_images/             # 校准数据集 (用于量化)
│   └── cat.*.jpg/png
│
...
│       └── model/          # 模型推理示例
│
├── mobilenetv2.onnx*       # MobileNetV2 模型
├── resnet18.onnx*          # ResNet18 模型
└── cat.png                 # 测试图片
```

> 💡 **说明**: 
> - `build/` 目录包含编译生成的所有输出文件
> - `my_ai_compiler/` 是编译器核心代码
> - `calib_images/` 用于 PTQ 量化的校准数据
> - `.onnx` 和 `.onnx.data` 是预训练的 ONNX 模型文件
